...
export function IPOCard({ ipo }: IPOCardProps) {
    ...
    return (
        <Card className="overflow-hidden transition-all hover:shadow-md hover:border-primary/50 group relative">
                    ...
                    <div>
                        <div className="flex items-center space-x-2">
                            ...
                            <Badge variant={isOpen ? 'default' : 'secondary'} className={isOpen ? 'bg-emerald-500/15 text-emerald-500 border-emerald-500/20 hover:bg-emerald-500/25' : ''}>
                                {isOpen ? 'Talep Topluyor' : ipo.status === 'upcoming' ? 'Yakında' : ipo.status}
                            </Badge>
                        </div>
